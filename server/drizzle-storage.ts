...
export class DrizzleStorage implements IStorage {
  ...
  async getSettings(): Promise<StorageSetting | undefined> {
    ...
    const mapSettings = (s: any): StorageSetting => ({
      ...
      licenseExpiry: s.licenseExpiry ?? undefined,
      licenseEmail: s.licenseEmail ?? undefined,
      licensePhone: s.licensePhone ?? undefined,
      licenseType: s.licenseType ?? undefined,
      ...
      menuBackgroundVideo: s.menuBackgroundVideo ?? undefined,
  ...
  async reorderCategories(items: { id: string; order: number }[]): Promise<void> {
    ...
    for (const { id, order } of items) {
      await db.update(categories).set({ order } as any).where(eq(categories.id, id));
    }
  }
  ...
  async reorderFoodTypes(items: { id: string; order: number }[]): Promise<void> {
    const db = getDb();
    for (const { id, order } of items) {
      await db.update(foodTypes).set({ order: String(order) } as any).where(eq(foodTypes.id, id));
    }
  }
  ...
  async reorderMaterials(items: { id: string; order: number }[]): Promise<void> {
    const db = getDb();
    for (const { id, order } of items) {
      await db.update(materials).set({ order: String(order) } as any).where(eq(materials.id, id));
    }
  }
